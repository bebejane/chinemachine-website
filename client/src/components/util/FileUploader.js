...
class FileUploader extends Component {
	...
	async uploadFile(file, fileIndex, filesTotal) {
		console.log(file);
		return new Promise((resolve, reject) => {
			...
			if (file) {
				...
				reader.onprogress = (e) => {
					...
					if (this.props.onUploadProgress) this.props.onUploadProgress(prog);
				};
				reader.onerror = (err) => {
					reject(err);
	...
	async uploadFiles(files) {
		...
		const f = [];
		try {
			for (var i = 0; i < files.length; i++) f.push(await this.uploadFile(files[i], i + 1, files.length));
	...
	renderFileLimitExceeded() {
		let text = this.props.customLimitText
			...
			: 'File size exceeds ' + this.props.fileSizeLimit / 1000 + 'MB limit';
	...
	render() {
		const { progress, showDrop, id, style, backgroundColor, color, label } = this.state;
		return (
			<div
				ref={this.ref}
			...
			>
				<div
					...
					id={id}
					style={{
						opacity: showDrop || (progress && progress !== 100) ? 1 : 0,
						zIndex: showDrop || (progress && progress !== 100) ? 20000 : 0,
