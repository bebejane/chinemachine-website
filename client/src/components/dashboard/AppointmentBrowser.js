...
class AppointmentBrowser extends Component {
	...
	handleBrowse(ffw) {
		...
		const newDate = (ffw ? moment(date).add(1, 'weeks') : moment(date).subtract(1, 'weeks')).toDate();
	...
	async loadAppointments(date) {
		...
		try {
			...
			week = week.filter((ap) => ap.userId && ap.user && (ap.status === 'BOOKED' || ap.status === 'CANCELLED'));
											...
											{day.appointments.map((app, i) => {
												return (
													<tr key={'tr' + i}>
														<td
															className={
																app.status === 'CANCELLED' ? 'dash-appointment-cancelled' : 'dash-appointment-fullname'
															}
														>
															{app.user && capitalize(app.user.lastName) + ', ' + capitalize(app.user.firstName)}
														</td>
