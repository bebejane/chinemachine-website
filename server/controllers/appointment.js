...
const getAllByWeek = async (year, week) => {
	...
	const startDate = moment().year(parseInt(year)).isoWeek(parseInt(week)).hour(0).minute(0).second(0).isoWeekday(1);
...
const upsert = async (appointment) => {
	return !appointment._id ? await add(appointment) : await update(appointment._id, { ...appointment });
};
