...
router.post(
	"/user/:userId/appointment/:appointmentId/cancel",
...
router.post(
	"/user/:userId/appointment/:appointmentId/book",
	authorize(Roles.USER),
	async (req, res, next) => {
		const { userId, appointmentId } = req.params;
		try {
...
router.get("/util/count", async (req, res, next) => {
	try {
		res.json(await appointmentController.getActiveCount());
	} catch (err) {
		next(err);
	}
});
...
router.get(
	"/util/byweek/:year/:week",
	authorize([Roles.MANAGER, Roles.ADMINISTRATOR]),
	async (req, res, next) => {
...
router.get("/:year/:month/:date", async (req, res, next) => {
	...
	try {
		res.json(
			await appointmentController.getAllByDate(parseInt(year), parseInt(month), parseInt(date))
		);
	} catch (err) {
		next(err);
	}
...
router.get("/:year/:month/:date/store/:storeId", async (req, res, next) => {
	...
	try {
		res.json(
			await appointmentController.getByDate(
				parseInt(year),
				parseInt(month),
				parseInt(date),
				req.params.storeId
...
router.get(
	"/",
	authorize([Roles.EMPLOYEE, Roles.MANAGER, Roles.ADMINISTRATOR]),
	async (req, res, next) => {
		try {
